...
trait DBTest {
  ...
  def exec[T](cmd: DBIO[T]): T = Await.result(db.run(cmd), 2 seconds)
  def execFuture[T](future: Future[T]): T = Await.result(future, 2 seconds)
}

class DataAccessSpec extends FlatSpec with BeforeAndAfter with DBTest {
